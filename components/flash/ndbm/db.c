...
*/
#define IDXLEN_SZ   4   /* index record lenght (ASCII chars) */
...
/**
 * Library's private reprentation of the database.
*/
typedef struct {
    ...
    off_t ptroff;   /* chain ptr offset pointing to this record */
    off_t chainoff; /* offset of hash chain for this idx record */
...
DBHANDLE db_open(const char *pathname, int oflag, ...)
{
    ...
    */
    ...
    db->hashoff = HASH_OFF; /* offset in index file of hahs table */
    ...
    if (oflag & O_CREAT) {
        ...
        /**
         * Open index file and data files.
        */
    ...
    if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)) {
        /**
         * If the databasee was created, we have to initialize
...
/**
 * Allocatr & initialize a DB structure and its buffers.
*/
static DB *_db_alloc(int namelen)
{
    ...
    /**
     * Use calloc, to initialize the sttructure to zero.
    */
...
/**
 * Fetch a record. Return a pointer to the null-tetrminated data.
*/
char *db_fetch(DBHANDLE h, const char *key)
{
    ...
    /**
     * Unlock the hash chain that _db_find_lock locked.
    */
 ...
 * and db_store. Returns with the hahs chain locked.
*/
static int _db_find_and_lock(DB *db, const char *key, int writelock)
{
     ...
     * This is where our search starte. First we calculate the
    ...
    */
    ...
    /**
     * We lock the hash chain here. The ecaller must unlock it
    ...
    */
    ...
    /**
     * offset == 0 on error (record not fonud).
    */
    return (offset == 0 ? -1 : 0);
}
...
static DBHASH _db_hash(DB *db, const char *key)
{
    ...
    for (i = 1; (c = *key++) != 0; i++)
        hval += c * i;  /* ascii char times its l-based index */
...
static off_t _db_readptr(DB *db, off_t offset)
{
    ...
    if (lseek(db->idxfd, offset, SEEK_SET) == -1)
        err_dump("_db_readptr: lseek error to ptr fileld");
 ...
 * corresponding data record in the the data file.
*/
...
static void _db_dodelete(DB *db)
{
    ...
    */
    ...
    /**
     * Write the data record whit all blanks.
    */
     ...
     * the deleted record becomes the head of the list.
    */
...
/**
 * Write a data record. Called by _db_doblete (t0 write
                        ...
                        off_t offset, int whence, off_t ptrval)
{
    ...
    if ((db->ptrval = ptrval) < 0 || ptrval > PTR_MAX)
        err_quit("_db_writeidex: invalid ptr: %ld", ptrval);
    ...
    /**
     * If we're appending, we hava to lock before doing the lseek
     * and write to make the two an atomic operation. If we're
     * overwriting an existing record, we don't have to lock.
    */
    ...
    /**
     * Psoition the index file and record the offset.
    */
 ...
 * the free llst, the hash table, or in an index record.
*/
...
int db_store(DBHANDLE h, const char *key, const char *data, int flag)
{
    ...
    if (datlen < DATLEN_MIN || datlen > DATLEN_MAX)
        err_dump("_db_store: invalid data length");
    ...
    */
    if (_db_find_and_lock(db, key, 1) < 0) { /* record not found */
        ...
        /**
         *  _db_find_and_locked the hahs chain foe us; read
        ...
        */
        ...
        /**
         * We are replacing an existing record. We konw the new
         * key equals the existing key. but we need to check if
         * the data records are thr same size.
        */
        if (datlen != db->datlen) {
             ...
             * (it may change whit the deletion)。
            */
            ptrval = _db_readptr(db, db->chainoff);
...
static int _db_findfree(DB *db, int keylen, int datlen)
{
    ...
    */
    ...
    if (offset == 0) {
         ...
         * which sets dv->pteval. Also, saveoffset points to
        ...
        */
        ...
        /**
         * Noticr also that _db_readidx set both db->idxoff
         * and db->datoff. This is used by the caller, db_store,
         * to write thr new index record and data record.
        */
    }
...
/**
 * Rewinf the index file for db_nextrec.
 ...
 * records. db_rewind must be called before this functiofn is
...
char *db_nextrec(DBHANDLE h, char *key)
{
    ...
    /**
     * We read lock the free list so that we do'nt read a record in the middle if its being deleted.
    */
    ...
    do {
        ...
        */
        ...
        /**
         * Check if key id all balck (empty record).
        */
        ptr = db->idxbuf;
